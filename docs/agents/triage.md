...
First responder in the incident pipeline. Classifies alerts and routes genuine anomalies to the Investigator.

## Role

```mermaid
graph TD
    ALERT["Alert Payload"] --> PARSE["Parse Alert"]
    PARSE --> MEM["Check MEMORY.md<br/>Known patterns?"]
    MEM -->|"yes"| RESOLVE["Auto-resolve"]
    MEM -->|"no"| CLASSIFY["Classify Severity"]
    CLASSIFY -->|"critical"| ESCALATE["Delegate to Investigator"]
...
| New error spike         | metrics_1m           | Escalate to Investigator      |
| New latency breach      | metrics_1m           | Escalate to Investigator      |
