...
Adapted for TelemetryFlow: four specialised agents forming an autonomous incident response pipeline, powered by **40 plugin tools** covering all **20 TFO Platform modules**.
...
graph TD
    ...
    INV["<b>Investigator</b><br/>claude-sonnet-4-5 + 40 plugin tools — metrics, logs, traces, exemplars, K8s, DB, LLM"]
...
## RCA & Postmortem — 3 Reporting Tools

After remediation, the pipeline generates structured incident reports with zero manual effort.

| Tool                    | Output                                                                                               |
| ----------------------- | ---------------------------------------------------------------------------------------------------- |
| `generate_rca_report`   | Full Root Cause Analysis — 5W analysis, mermaid timeline, impact metrics, blast radius, action items |
| `generate_postmortem`   | Comprehensive postmortem — timeline, lessons learned, remediation flow, appendix                     |
| `generate_rca_template` | Blank template for manual human review — document control, signature section                         |

### Jira & Trello Integration

`generate_rca_report` can create Jira tickets and Trello cards alongside the RCA report. Submission is **generate-only by default** — dual-gated by:

1. **Master switch env vars**: `JIRA_ENABLED=true` / `TRELLO_ENABLED=true` (off by default)
2. **`--submit true` flag**: explicit opt-in per invocation

```
Actions:
  rca            → report only
  jira / trello  → report + ticket (generated, not submitted)
  jira-submit / trello-submit / submit → force submit regardless of flags
```
...
- **Mermaid diagrams**: incident timeline, response flow, causal chain, blast radius
- **5W analysis**: What, Where, When, Why, How — with ClickHouse evidence citations
- **Impact metrics**: before/during/after with actual metric values
- **Action items**: owners, priorities, due dates
- **Lessons learned**: what went well, what to improve, where lucky

Reports save to `~/.hermes/reports/` and are auto-attached to the post-remediation hook.
...
## TelemetryFlow Plugin Tools — 40 Tools, 20 Modules
...
| **Infrastructure (5)**   | `manage_retention`, `manage_tenancy`, `manage_iam`, `manage_sso`, `query_tfql`                                                                        |
| **Remediation (4) ⚠**    | `scale_deployment`, `restart_pod`, `rollback_deploy`, `update_alert` — all require human approval                                                     |
| **RCA & Postmortem (3)** | `generate_rca_report`, `generate_postmortem`, `generate_rca_template` — 5W analysis, mermaid diagrams, Jira/Trello integration                        |
...
| **Tool Permissions** | 40 plugin tools, 4 gated ⚠                                      | Unauthorized write access       |
...
| **Cyber Defense**    | Threat-informed triage + security review on every investigation | Security blind spots            |
...
```mermaid
graph LR
    HERMES["tfo-hermes<br/>40 Plugin Tools"] --> API["TFO Backend<br/>:3000"]
    API --> PG["PostgreSQL"]
    API --> CH["ClickHouse"]
    API --> RD["Redis"]
    API --> NATS["NATS"]
...
| `monitoring` | TFO Collector + TFO Agent                                 |
| `tools`      | Portainer                                                 |
...
| RCA Reports           | `generate_rca_report`           | 5W analysis, mermaid timeline, Jira/Trello     |
| Postmortem            | `generate_postmortem`           | Lessons learned, action items, timeline        |
| RCA Template          | `generate_rca_template`         | Blank template for manual human review         |
...
├── plugins/              # TelemetryFlow plugin (40 tools)
│   └── telemetryflow/
...
│           ├── query_metrics.py
│           ├── search_logs.py
│           ├── ...       # 40 tools total covering all 20 TFO modules
...
3. **The integration is plugin tools + API** — Hermes queries TelemetryFlow through 40 dedicated plugin tools covering all 20 TFO Platform modules. No direct ClickHouse connections — everything goes through the authenticated REST API.
...
> The integration is 40 plugin tools + TFO REST API.
